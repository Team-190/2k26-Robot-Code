...
public class V1_GammaIntakeConstants {
  ...
  public static final FourBarLinkageConstants LINKAGE_CONSTANTS =
    FourBarLinkageConstants.builder().CANCODER_SENSOR_DIRECTION(CANCODER_SENSOR_DIRECTION).CAN_CODER_CAN_ID(CAN_CODER_CAN_ID)
    .CAN_CODER_CAN_ID(CAN_CODER_CAN_ID).CONSTRAINTS(CONSTRAINTS).DRIVE_CONFIG(DRIVE_CONFIG).GAINS(GAINS).GEAR_RATIO(GEAR_RATIO).INTAKE_ANGLE_OFFSET(INTAKE_ANGLE_OFFSET)
    .IS_CAN_FD(IS_CAN_FD).LINKAGE_OFFSET(LINKAGE_OFFSET).LINK_BOUNDS(LINK_BOUNDS).LINK_CONST(LINK_CONST).LINK_LENGTHS(LINK_LENGTHS).MAX_ANGLE(MAX_ANGLE).MIN_ANGLE(MIN_ANGLE)
    .MOMENT_OF_INERTIA(MOMENT_OF_INERTIA).MOTOR_CAN_ID(MOTOR_CAN_ID).MOTOR_CONFIG(MOTOR_CONFIG).PIN_LENGTH(PIN_LENGTH).STATOR_CURRENT_LIMIT(STATOR_CURRENT_LIMIT)
    .SUPPLY_CURRENT_LIMIT(SUPPLY_CURRENT_LIMIT).ZERO_OFFSET(ZERO_OFFSET).build();
}
