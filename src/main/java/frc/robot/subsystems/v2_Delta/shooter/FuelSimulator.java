...
public class FuelSimulator {
    ...
    private static final Rotation2d hoodPitch = hoodIO.getPosition(); // Replace with actual value from log
    ...
    public void fireShot(Pose2d robotPose, ChassisSpeeds chassisSpeeds, Rotation2d hoodPitch, double flywheelSurfaceSpeed) {
    ...
    private boolean checkHubCollision(SimulatedFuel fuel, Rotation2d hoodPitch) {
        ...
        double FUEL_X_FINAL = TURRET_X_OFFSET + 
                    INITIAL_VELOCITY_X * Math.cos(hoodPitch.getRadians()) * Math.sqrt(Math.pow(INITIAL_VELOCITY_X * Math.sin(hoodPitch.getRadians()), 2) + 
                    19.6 * (HEIGHT_ROBOT - FieldConstants.Hub.innerHeight)) / 9.8;
