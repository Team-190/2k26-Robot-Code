...
public class LinkageIOTalonFX implements LinkageIO {
    ...
    public void updateInputs(LinkageIOInputs inputs) {
        ...
        inputs.rightPositionGoal = Rotation2d.fromRotations(rightPositionGoalRotations.getValueAsDouble());
    ...
    public void setVoltageLeft(double volts) {
        leftTalonFX.setControl(voltageControlRequest.withOutput(volts).withEnableFOC(true).withUpdateFreqHz(100.0));
    }

    @Override
    public void setVoltageRight(double volts) {
        rightTalonFX.setControl(voltageControlRequest.withOutput(volts).withEnableFOC(true).withUpdateFreqHz(100.0));
    }
    ...
    public boolean atGoalLeft() {
        return Math.abs(Units.rotationsToRadians(leftPositionErrorRotations.getValueAsDouble())) <= LinkageConstants.CONSTRAINTS.goalToleranceRadians().get();
    }

    @Override
    public boolean atGoalRight() {
        return Math.abs(Units.rotationsToRadians(rightPositionErrorRotations.getValueAsDouble())) <= LinkageConstants.CONSTRAINTS.goalToleranceRadians().get();
    }
}
