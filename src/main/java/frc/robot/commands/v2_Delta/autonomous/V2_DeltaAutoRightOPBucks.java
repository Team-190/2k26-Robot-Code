...
public class V2_DeltaAutoRightOPBucks {
  public static Command getAutoRoutine(
    ...
    try {
      PathPlannerPath OP_1_CROSS = PathPlannerPath.fromPathFile("OP_1_CROSS").mirrorPath();
      PathPlannerPath OP_2 = PathPlannerPath.fromPathFile("OP_2").mirrorPath();

      AdjustPathCommand followCommandOP_1_CROSS =
      ...
      return Commands.sequence(
          ...
          followCommandOP_1_CROSS.onlyWhile(() -> {
                    Pose2d currentPose = V2_DeltaRobotState.getGlobalPose();
                    Pose2d targetPose = OP_1_CROSS.getPathPoses().get(OP_1_CROSS.getPathPoses().size() - 1);
