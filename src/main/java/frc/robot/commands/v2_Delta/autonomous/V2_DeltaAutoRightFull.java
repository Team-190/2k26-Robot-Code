  ...
  public static Command getAutoRoutine(
    ...
    try {
      AutoBuilder.configure(
          ...
          drive);
      RIGHT_FULL = PathPlannerPath.fromPathFile("LEFT_FULL_SWEEP").mirrorPath();
      return Commands.parallel(
          Commands.sequence(
              Commands.runOnce(
                  () ->
                      V2_DeltaRobotState.resetPose(
                          AllianceFlipUtil.apply(RIGHT_FULL.getStartingHolonomicPose().get()))),
              intake
                  .deploy()
                  .alongWith(intake.setOverrideRollerVoltage(IntakeConstants.INTAKE_VOLTAGE)),
              AutoBuilder.followPath(RIGHT_FULL),
