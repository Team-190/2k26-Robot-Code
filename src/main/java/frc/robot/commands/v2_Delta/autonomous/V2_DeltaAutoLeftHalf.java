  ...
  public static Command getAutoRoutine(
    ...
    try {
      AutoBuilder.configure(
          ...
          drive);
      LEFT_HALF = PathPlannerPath.fromPathFile("LEFT_HALF_SWEEP");
      return Commands.parallel(
          Commands.sequence(
              Commands.runOnce(
                  () ->
                      V2_DeltaRobotState.resetPose(
                          AllianceFlipUtil.apply(LEFT_HALF.getStartingHolonomicPose().get()))),
              intake
                  .deploy()
                  .alongWith(intake.setOverrideRollerVoltage(IntakeConstants.INTAKE_VOLTAGE)),
              AutoBuilder.followPath(LEFT_HALF),
          ...
          Commands.sequence(
              ...
              V2_DeltaCompositeCommands.hold(clopper, shooter).withTimeout(2.5),
